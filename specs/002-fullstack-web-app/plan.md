...
- **Backend**: FastAPI with SQLModel (async SQLAlchemy-based ORM) and JWT verification via JWKS
...
**Storage**: PostgreSQL 14+ with SQLModel with async support
...
| Session refresh | Long-lived session cookie (7-14 days) allows JWT refresh without re-authentication |
...
| SQLModel \| Type-safe, FastAPI-native ORM built on SQLAlchemy 2.0 with async support, migrations via Alembic |
...
2. Set up PostgreSQL connection with SQLModel (async)
...
2. Configure Drizzle ORM database adapter with PostgreSQL
